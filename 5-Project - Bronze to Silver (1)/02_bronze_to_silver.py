...
# TODO
rawToBronzeWriter = batch_writer(transformedRawDF,'p_ingestdate')
...
# TODO
bronzeDF = spark.read.table('health_tracker_classic_bronze').filter(col('status')=="new")
...
bronzeAugmentedDF = bronzeDF.withColumn('nested_json',from_json(bronzeDF.value,json_schema))
...
# TODO
silver_health_tracker = bronzeAugmentedDF.select(bronzeAugmentedDF.value.alias('value'),'nested_json.*')

# COMMAND ----------

display(silver_health_tracker)

# COMMAND ----------

type(silver_health_tracker)
...
silver_health_tracker = silver_health_tracker.select(col('value').alias('value'),col('device_id').cast('int').alias('device_id'),
                                                     col('steps').alias('steps'),col('time').alias('eventtime'),col('name').alias('name'),to_date(col('time')).alias('p_eventdate')
...
# TODO
silver_health_tracker_clean.select('device_id','steps','eventtime','name','p_eventdate').write.format('delta').mode('append').partitionBy('p_eventdate').save(silverPath)
