...
raw_health_tracker_data_df = (
  spark.read.format('text').option('inferSchema',kafka_schema).load(rawPath)
)
...
type(raw_health_tracker_data_df)
...
raw_health_tracker_data_df = (
  ...
  lit('files.training.databricks.com').alias('datasource'),current_timestamp().alias('ingesttime'),'value',lit('new').alias('status'), current_date().alias('p_ingestdate')
  )
)
...
dbutils.fs.rm('/dbacademy/XH/dataengineering/classic/bronze/',True)
...
(
  raw_health_tracker_data_df.write.format('delta').option('inferSchema',True).mode('append').partitionBy('p_ingestdate').save(bronzePath)
...
# MAGIC ##### one of the device id is very long and strange
